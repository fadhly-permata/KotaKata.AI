...
| **PLAN-040** | ✅ done | 5 tema baru: Glassmorphism, Claymorphism, Neo-Brutalism, Minimalist UI, Frost UI | 5 tema aplikasi baru di Pasar (registry APP_THEMES) — tiap tema: palet light/dark penuh, token skin `radius`+`shadow` (glass/frost=permukaan rgba transparan + bayangan lembut; clay=pastel hangat radius besar + highlight; brutalist=border hitam tebal + bayangan keras blur 0 satu arah; minimal=flat tanpa shadow), **board & keyboard theme id sama** (PLAN-033), SoundSpec & backsound ambient khas tiap tema (URL Mixkit terverifikasi); themes.sql diregenerate (34 tema) & di-push ke Supabase (HTTP 201); tsc + 43 tes + lint lolos |
| **PLAN-041** | ✅ done | Hapus kata vulgar (aman anak), ganti agar tiap tier tetap 1000 soal | 6 langkah: **daftar kata vulgar** (`scripts/vocab/vulgar-words.mjs` — 3 kelompok: VULGAR / ANSWER-ONLY / KONTEKSTUAL, word-boundary anti false-positive) + **scan jawaban & clue semua tier**; **52 jawaban diganti** kurasi (idiot/tolol/ganja/banci/cabul/mesum/dungu/kafir/bugil/anus/penis/opium/dadah, sialan/kentut/heroin/mampus/sundal/kunyuk/goblok/morfin/kontol/vagina/ereksi/testis, keparat/pelacur/bangsat/sinting/kampret/lesbian/ekstasi/mani/kelamin, bajingan/meniduri/narkotik/kemaluan, pelacuran/mariyuana/selingkuh, masturbasi/bersetubuh/pornografi/pencabulan, percabulan, fornikasi/gerempang/datatamak/apomorfin, testikel/klitoris — pengganti dipastikan tidak ada di tier mana pun) + **8 clue ditulis ulang** (lian/karat/kotoran/bermoral/kecanduan/bentangkan/argirodit/disentri + perpetuasi); **validasi**: scan ulang **0 vulgar / 0 answer-only**, gen-vocab-sql 1000/tier & tanpa dedup, check-clue-quality 0 issue semua file, audit-vocab 0 issue, tsc + 43 tes + lint lolos; **push ke Supabase** (HTTP 201, 1000/tier) + script QA `scripts/db/check-vulgar-db.mjs` → DB 0 kata vulgar |
...
| **PLAN-042** | ⏭ skipped | Review soal tier 2–10 | Di-skip pemilik 2026-08-16 (belum dikerjakan — ditampilkan hanya saat diminta) |
| **PLAN-043** | ✅ done | Popup dialog dibuat solid/opaque agar teks terbaca (fix glass/frost PLAN-040) | 4 langkah: **helper `solidSurfaceColor` di `skin.ts`** (rgba permukaan glass/frost di-blend di atas warna latar → warna solid) + **`surfaceStyleOpaque`**; diterapkan ke kartu dialog: AppModal (semua popup), ConfirmDialog, CompletionOverlay, sheet detail kata (History), modal hapus akun (Profil), kartu error AI (MainMenu) — teks dialog tidak lagi tembus pandang menimpa objek di belakangnya; efek glass/frost tetap di elemen halaman (bukan dialog); tsc + 43 tes + lint lolos |
| **PLAN-044** | ✅ done | Animasi latar mengikuti suasana backsound (hujan/angin/dll) tanpa membebani performa | 4 langkah: **petakan suasana → animasi** — tiap tema aplikasi diberi `ambientFx` (`rain | wind | embers | fireflies | waves | breeze | none`) sesuai backsound-nya; **komponen ringan `AmbientFx`** (partikel ≤12, satu `Animated.Value` per partikel + native driver, tanpa re-render tiap frame, `pointerEvents="none"`, loop diam-diam, warna palet tema); **wire ke `ScreenFade`** (menggantikan orb saat tema punya `ambientFx`; Main Menu & Login tetap punya FloatingOrbs sendiri — tidak dobel); **performa** — hanya berjalan saat layar fokus (`useIsFocused`, pola mitigasi force close PLAN-023/024/027); tsc + 43 tes + lint lolos |
| **PLAN-046** | ✅ done | XP ≥ 800.000: semua kata dari tier manapun boleh keluar lagi | 4 langkah: **konstanta `UNLOCK_ALL_TIERS_XP = 800000`** + helper `shouldUnlockAllTiers(totalXp)` di `xpEngine.ts`; **`vocabularyRepository.getAllTiersFromCloud(excludedWordIds)`** — fetch pool dari tier 1–10 tanpa filter tier, kata yang sudah ditemukan tetap dikecualikan server-side (`not.in` di-chunk); **wire** — `WordPoolFilterParams.allTiers` + `selectWordPool` branch (all-tiers vs per-tier), `GameScreen` mengirim flag dari `totalXp` saat generate papan normal; **3 tes unit baru** (bawah/tepat/atas ambang + ambang 800k > tier 10 500k); tsc + 50 tes + lint lolos |
| **PLAN-049** | ✅ done | Mode AI — tampilkan informasi bahwa bahasa soal adalah Bahasa Indonesia | Info 🇮🇩 ditambahkan di **halaman Pengaturan Provider AI** (baris "Semua soal, kosakata, dan petunjuk dibuat dalam Bahasa Indonesia") + **dialog loading "Menyusun soal dari AI…"** di Main Menu (baris "Soal dibuat dalam Bahasa Indonesia"); badge in-game opsional dilewati (header sempit); tsc + 47 tes + lint lolos |
| **PLAN-050** | ✅ done | Mode AI — exclude kata yang sudah pernah ditemukan/dipecahkan pemain | 4 langkah: **`requestAiWords(cfg, tier, excludeWords[], signal)`** — exclude dinormalisasi (trim + lower-case, cap **300**) → **dilarang di prompt AI** ("JANGAN gunakan kata-kata berikut…") **dan difilter lagi di sisi klien** (jaga-jaga model bandel); **`handlePlayAi`** ambil riwayat kata ditemukan (`getDiscoveredWordIds` dukung limit 300 terbaru + `getByIds` → teks kata; gagal ambil riwayat = lanjut tanpa exclude, Mode AI tidak terblokir); **4 tes unit baru** (exclude di prompt, filter klien, cap 300, tanpa exclude); tsc + 47 tes + lint lolos |
| **PLAN-051** | ✅ done | Fix animasi orb sering macet di web (setelah patch terakhir) | 2 akar masalah: **(1) churn fokus** — pola `loop.stop()` → `setValue(0)` → loop baru saat ganti fokus bermasalah di react-native-web; **(2) churn render** — array `orbBounce` dibuat INLINE di body komponen (MainMenu & Auth) sehingga identitasnya berubah tiap render → effect animasi stop/restart SEMUA loop setiap render (inilah penyebab utama macet; tetap ada setelah fix tahap 1 karena `useMemo` bergantung pada array tak stabil itu). **Fix**: helper `useAmbientLoops` (`src/utils/ambientLoop.ts`) — **web = loop sekali mount** (deps `[]`, kebal churn fokus & render), **native = tetap gated fokus** (mitigasi force close); **array `orbBounce` distabilkan via `useRef`** (dibuat sekali); diterapkan ke `AmbientOrbs`, `MainMenuScreen` (bounce + 7 orb), `AuthScreen` (4 orb); `AmbientFx` platform-split inline; `CompletionOverlay` aman (loop sekali mount); tsc + 47 tes + lint lolos |
...
### v1.3.12 — XP 800.000: Buka Semua Kata Semua Tier (PLAN-046)

- 🏆 **Unlock semua tier di XP 800.000 (PLAN-046)** — pemain dengan total XP **≥ 800.000** (jauh melampaui ambang tier 10 = 500.000) tidak lagi dibatasi tier saat generate papan: pool kata diambil dari **SEMUA tier (1–10)**. Kata yang sudah pernah ditemukan tetap dikecualikan dari tier mana pun (aturan `word_discoveries` tidak berubah).
- ⚙️ **Implementasi** — konstanta `UNLOCK_ALL_TIERS_XP = 800000` + `shouldUnlockAllTiers()` di `xpEngine.ts`; method baru `vocabularyRepository.getAllTiersFromCloud(excludedWordIds)` (eksklusi kata ditemukan tetap server-side, `not.in` di-chunk); `selectWordPool` menerima flag `allTiers` dan `GameScreen` mengirimnya dari XP user.

### v1.3.11 — Mode AI: Info Bahasa Indonesia, Exclude Kata Ditemukan & Fix Animasi Orb Web (PLAN-049 + PLAN-050 + PLAN-051)

- 🇮🇩 **Mode AI: info Bahasa Indonesia (PLAN-049)** — pemain kini tahu soal Mode AI selalu berbahasa Indonesia: ada baris info "🇮🇩 Semua soal, kosakata, dan petunjuk dibuat dalam Bahasa Indonesia" di halaman **Pengaturan Provider AI**, dan "🇮🇩 Soal dibuat dalam Bahasa Indonesia" di **dialog loading** saat menyusun soal dari provider.
- 🚫 **Mode AI: exclude kata yang sudah ditemukan (PLAN-050)** — kata yang pernah lo pecahkan tidak akan keluar lagi sebagai soal AI: sebelum meminta soal, app mengambil riwayat kata ditemukan (300 kata terbaru dari cloud), melarangnya di prompt AI, **dan** menyaring lagi hasilnya di sisi klien (jaga-jaga model melanggar). Kalau riwayat gagal dimuat (offline/error), Mode AI tetap jalan tanpa exclude — tidak pernah terblokir.
- 🐛 **Fix animasi orb macet di web (PLAN-051)** — orb (dan partikel backsound) yang sering berhenti/diam di web diperbaiki. Ada **dua penyebab**: (1) pola stop/restart loop saat ganti fokus yang bermasalah di react-native-web, dan (2) — penyebab utama yang baru ketahuan — array `orbBounce` dibuat **inline di body komponen** (Main Menu & Login) sehingga identitasnya berubah **setiap render** → semua loop animasi di-stop lalu di-restart tiap kali layar re-render, dan di web restart yang berulang itu bikin value animasi macet di posisi akhir (orb diam). Kini loop web dijalankan **sekali saat halaman terbuka** (helper `useAmbientLoops` dengan deps `[]` — kebal churn fokus maupun render) dan array `orbBounce` **distabilkan lewat `useRef`** (dibuat sekali). **Android tetap pakai guard fokus** supaya mitigasi force close (PLAN-023/024/027) tidak rusak. Diterapkan ke semua lokasi: `AmbientOrbs`, orb & bounce Main Menu, orb Login, dan partikel `AmbientFx`.

### v1.3.10 — Dialog Solid & Animasi Latar Mengikuti Backsound (PLAN-043 + PLAN-044)

- 🪟 **Dialog tidak lagi transparan (PLAN-043)** — kartu dialog kini memakai permukaan **solid/opaque**: helper baru `solidSurfaceColor()` di `skin.ts` mem-blend warna surface glass/frost (rgba) di atas warna latar menjadi warna solid, dipakai `surfaceStyleOpaque()`. Diterapkan ke **semua** popup: AppModal (Kata Ajaib, Daftar Tier, Leaderboard, error Mode AI, dst.), ConfirmDialog (keluar/reset), CompletionOverlay (permainan selesai), sheet detail kata di Sejarah, modal hapus akun di Profil, dan kartu error AI di Main Menu. Teks di dalam dialog selalu terbaca — tidak lagi menimpa/tumpang tindih objek di belakangnya. Efek kaca glass/frost tetap di elemen halaman (permukaan bukan dialog).
- 🌧 **Animasi latar mengikuti suasana backsound (PLAN-044)** — tema dengan suasana spesifik kini menggantikan orb dengan **partikel ambien yang cocok dengan suara latarnya**: hujan → **rintik hujan turun**, angin → **garis angin melintas**, api unggun → **bara/percikan naik**, jangkrik malam → **titik kelap-kelip**, ombak → **gelombang lembut**, angin sepoi → **dedaunan melayang**.
- ⚡ **Ringan & aman untuk perangkat tua** — komponen baru `AmbientFx`: partikel **≤12**, tiap partikel satu `Animated.Value` dengan **native driver** (tanpa re-render tiap frame), `pointerEvents="none"`, warna dari palet tema aktif, dan **hanya berjalan saat layar fokus** (`useIsFocused`) — mengikuti pola mitigasi force close PLAN-023/024/027. Main Menu & Login tetap memakai FloatingOrbs sendiri (tidak dobel).

### v1.3.9 — Kosakata Bersih & Aman Anak (PLAN-041)

- 🧹 **52 kata vulgar/tidak layak dihapus dari semua tier** (makian, alat kelamin, tindakan seksual, hinaan, narkoba) — diganti kata kurasi berkesulitan setara dengan 3 clue baru yang tidak bocor jawaban (mis. idiot→pandir, ganja→hening, kontol→sentosa, pelacur→pemandangan, ganja/morfin/heroin/ekstasi/mariyuana/dadah→kata alam & suasana, dst.).
- ✏️ **8 clue yang memuat kata vulgar ditulis ulang** (mis. "tahi besi"→"lapisan cokelat kemerahan", "Antonim: bejat"→"Antonim: amoral", kutipan "berzina"/"bajingan" dibersihkan).
- 🛡 **Scanner permanen** `scripts/vocab/vulgar-words.mjs` (3 kelompok: vulgar wajib, jawaban tidak layak, kata kontekstual ditinjau manual) + **QA DB** `scripts/db/check-vulgar-db.mjs` — hasil scan: **0 vulgar / 0 jawaban tidak layak**, DB Supabase: **0 kata vulgar**, tetap **1000 soal per tier**.

### v1.3.8 — 5 Tema Baru (PLAN-040)

- 🪟 **Glassmorphism** — latar gradien hidup (ungu→biru→merah muda) yang menembus permukaan kaca semi-transparan (`rgba` putih) dengan border tipis bercahaya & bayangan lembut menyebar; backsound "Angin sepoi", sound efek "Jernih".
- 🏺 **Claymorphism** — pastel hangat (krim/peach/mawar), permukaan "tanah liat" dengan sudut sangat membulat (radius 22) + highlight terang & bayangan lembut berwarna; backsound "Api unggun malam", sound "Hangat".
- 🟥 **Neo-Brutalism** — warna solid berani, border hitam tebal, sudut nyaris kotak (radius 4), dan **bayangan keras offset tanpa blur** (satu arah — trik `light: transparent` di spec shadow); versi gelap memakai border & bayangan putih; backsound "Hiruk-pikuk kota", sound "Tegas".
- ⬜ **Minimalist UI** — flat total (tanpa token shadow → semua permukaan datar), latar polos, border abu halus, aksen hitam-putih; backsound "Angin hutan sejuk", sound "Hening".
- 🧊 **Frost UI (soft glass)** — variasi glass lebih lembut: kaca buram es biru rendah saturasi, bayangan sangat halus, border tipis seperti embun beku; backsound "Angin dingin", sound "Dingin".
- 🎨 Kelima tema punya **papan & keyboard senama** (mengikuti tema aplikasi, PLAN-033) dan terlihat di **Pasar** (katalog Supabase sudah di-update — 34 tema total).
