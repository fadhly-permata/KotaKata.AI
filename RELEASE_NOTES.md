...
- 🚫 **Aturan GitHub Release** — release tidak lagi dibuat dari workspace ini: perintah `gh` memakai kredensial GitHub App Freebuff, sehingga rilis yang dibuat lewat `gh release create` tercatat atas nama `freebuff-web[bot]` (bukan pemilik). Aturan baru di `AGENTS.md`: kalau perlu release, siapkan draf catatan rilis + tag dan minta pemilik publish dari akun GitHub-nya sendiri
