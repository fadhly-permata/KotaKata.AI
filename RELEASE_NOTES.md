...
| **PLAN-051** | ✅ done | Fix animasi orb sering macet di web (setelah patch terakhir) | Akar masalah: pola `loop.stop()` → `setValue(0)` → loop baru saat layar ganti fokus (guard PLAN-023/024/027) bikin react-native-web macet di posisi akhir. **Helper `useAmbientLoops`** (`src/utils/ambientLoop.ts`): **web = loop sekali mount tanpa churn stop/restart** (anti macet), **native = tetap gated fokus** (mitigasi force close). Diterapkan ke `AmbientOrbs`, `MainMenuScreen` (bounce tombol main + 7 orb), `AuthScreen` (4 orb); `AmbientFx` pakai platform-split inline (custom loop partikel); `CompletionOverlay` aman (loop sekali mount); tsc + 47 tes + lint lolos |
...
- 🐛 **Fix animasi orb macet di web (PLAN-051)** — orb (dan partikel backsound) yang sering berhenti/diam di web diperbaiki: penyebabnya pola stop/restart loop saat ganti fokus yang bermasalah di react-native-web. Kini di web loop dijalankan sekali saat halaman terbuka (tanpa churn stop/restart — tidak macet), sedangkan di **Android guard fokus tetap dipertahankan** supaya mitigasi force close (PLAN-023/024/027) tidak rusak. Diterapkan ke semua lokasi: `AmbientOrbs`, orb & bounce Main Menu, orb Login, dan partikel `AmbientFx`.
