...
const CACHE = "/tmp/kotakata-research-cache";
...
const cats = { paren: 0, abbrev: 0, marker: 0, ph: 0, c1Bad: 0, leakPat: 0, short: 0, leak: 0 };
const noKbbi = [];
const withKbbi = [];
let total = 0;
...
for (const f of files.sort()) {
  ...
  for (const line of lines) {
    ...
    for (const [k, v] of Object.entries(flags)) if (v) cats[k]++;
    const hasKbbi = existsSync(join(CACHE, `${w}.json`));
    const entry = { f, w, flags, cache: hasKbbi };
    if (hasKbbi) withKbbi.push(entry);
    else noKbbi.push(entry);
  }
}
console.log("Total rows:", total);
console.log("Categories:", JSON.stringify(cats));
console.log("Issues with research cache (potential material):", withKbbi.length);
console.log("Issues WITHOUT cache (definitely no KBBI entry):", noKbbi.length);
console.log("\n--- Sample with cache but still broken ---");
for (const x of withKbbi.slice(0, 25)) {
  console.log(`${x.f} ${x.w} ${JSON.stringify(x.flags)}`);
}
